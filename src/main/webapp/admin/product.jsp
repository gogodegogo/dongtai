...
<div id="brall">
    ...
    <div id="condition" style="text-align: center">
        <form id="myform">
            员工姓名：<input name="ename" id="ename">&nbsp;&nbsp;&nbsp;
            职位：<select name="typeid" id="typeid">
            <option value="-1">请选择</option>
            <c:forEach items="${Tpyelist}" var="pt">
                <option value="${pt.type}">${pt.zhiwei}</option>
            </c:forEach>
        </select>&nbsp;&nbsp;&nbsp;
            价格：<input name="minmoney" id="minmoney">-<input name="maxmoney" id="maxmoney">
                ...
                <div id="middle">
                    <table class="table table-bordered table-striped">
                        ...
                        <c:forEach items="${info.list}" var="p">
                            <tr>
                                <td valign="center" align="center"><input type="checkbox" name="ck" id="ck" value="${p.empno}" onclick="ckClick()"></td>
                                <td>${p.ename}</td>
                                <td>${p.job}</td>
                                <td>${p.empno}</td>
                                <td><img width="55px" height="45px"
                                         src="${pageContext.request.contextPath}/image_big/${p.image}"></td>
                                <td>${p.sal}</td>
                                ...
                                <td>
                                    <c:forEach items="${Tpyelist}" var="emptype">
                                        <c:if test="${p.type == emptype.type}">
                                            ${emptype.zhiwei}
                                        </c:if>
                                    </c:forEach>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-info "
                                            onclick="one(${p.empno},${info.pageNum})">编辑
                                    </button>
                                    <button type="button" class="btn btn-warning" id="mydel"
                                            onclick="del(${p.empno})">删除
                                    </button>
                                </td>
                            </tr>
                        </c:forEach>
...
<script type="text/javascript">
    ...
    function one(empno, pageNum) {
        var ename = $("#ename").val();
        var typeid = $("#typeid").val();
        var minmoney = $("#minmoney").val();
        var maxmoney = $("#maxmoney").val();
        var str = "empno="+empno+"&ename="+ename+"&typeid="+typeid+"&minmoney="+minmoney+"&maxmoney="+maxmoney+"&page="+pageNum;
...
<script type="text/javascript">
    function ajaxsplit(page) {
        ...
        var maxmoney = $("#maxmoney").val();
        $.ajax({
            url:"${pageContext.request.contextPath}/prod/chaxu.action",
            data:{"page":page,"ename":ename,"typeid":typeid,"minmoney":minmoney,"maxmoney":maxmoney},
            type:"post",
            success:function () {
                //重新加载分页显示的组件table
